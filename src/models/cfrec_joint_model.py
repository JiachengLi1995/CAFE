...
import torch
from torch import logit, nn as nn
...
class CFRecJointModel(nn.Module):
    ...
    def forward(self, 
        ...
        if mode == "train":
            ...
            meta_candidates = meta_candidates[idx1, idx2]

            pos_seqs = candidates[:, 0]
            neg_seqs = candidates[:, -1]

            meta_pos_seqs = meta_candidates[:, 0]
            meta_neg_seqs = meta_candidates[:, -1]

            pos_embs = self.product_embedding.lookup_target(pos_seqs)
            neg_embs = self.product_embedding.lookup_target(neg_seqs)

            meta_pos_embs = self.category_embedding.lookup_target(meta_pos_seqs)
            meta_neg_embs = self.category_embedding.lookup_target(meta_neg_seqs)

            pos_logits = (feature * pos_embs).sum(dim=-1)
            neg_logits = (feature * neg_embs).sum(dim=-1)

            meta_pos_logits = (meta_feature * meta_pos_embs).sum(dim=-1)
            meta_neg_logits = (meta_feature * meta_neg_embs).sum(dim=-1)
            ...
            loss += self.loss(meta_pos_logits, pos_labels)
            loss += self.loss(meta_neg_logits, neg_labels)
            ...
            if candidates is not None:
                ...
                logits = self.product_embedding.all_predict(feature) # (batch_size, item_num)

            if meta_candidates is not None:
                log_feats = meta_feature.unsqueeze(1) # x is (batch_size, 1, embed_size)
                w = self.category_embedding.lookup_target(meta_candidates).transpose(2,1) # (batch_size, embed_size, candidates)
                logits_meta = torch.bmm(log_feats, w).squeeze(1) # (batch_size, candidates)
            else:
                logits_meta = self.category_embedding.all_predict(meta_feature) # (batch_size, meta_num)
