...
class SelfAttention(nn.Module):
    ...
    def forward(self, seqs, side_seqs, timeline_mask):

        seqs *= ~timeline_mask.unsqueeze(-1) # broadcast in last dim
        side_seqs *= ~timeline_mask.unsqueeze(-1)
        side_seqs = torch.transpose(side_seqs, 0, 1)
        ...
        for i in range(len(self.attention_layers)):
            ...
            fusion_seqs = seqs + side_seqs
            Q = self.attention_layernorms[i](fusion_seqs)
            mha_outputs, attn_output_weight = self.attention_layers[i](Q, fusion_seqs, seqs, 
...
class Embedding(nn.Module):
    def __init__(self, num_items, args, pad_token, pretrained_item_vectors=None, use_pretrained_vectors=False):
        super().__init__()
...
class NovaModel(nn.Module):
    ...
    def forward(self, tokens, meta_tokens=None, candidates=None, meta_candidates=None, length=None, mode="train", need_weights=False, users=None):
        ...
        feature, attn_weights = self.product_attention(feature, meta_feature, time_mask)
